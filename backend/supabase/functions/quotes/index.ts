...
async function fetchHistory(symbol: string, range: string) {
  if (!symbol || symbol === 'MEP' || symbol === 'CCL') return { symbol, range, points: [] }
  const res = await cached(`hist:${symbol}:${range}`, () => bymaHistory(symbol, range), HISTORY_CACHE_TTL_MS)
...
async function fetchQuotes(symbols: string[]) {
  const values = await mapWithConcurrency(symbols, 2, (symbol) =>
    resolveWithRetry(symbol).catch(() => null),
  )
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (typeof body?.history?.symbol === 'string') {
      ...
      history = { symbol: h, range: typeof body.history.range === 'string' ? body.history.range : '' }
    }
  } catch {
